...
function Translate() {
    ...
    const handleChange = (lang: string | undefined) => {
            ...
            <div className="container d-flex flex-wrap justify-content-end">
                <div className="dropdown">
                    ...
                    <ul className="dropdown-menu" aria-labelledby="dropdownMenuLink">
                        <li><button className="dropdown-item" onClick={e => { handleChange('en') }}>{t('header.english')}</button></li>
                        <li><button className="dropdown-item" onClick={e => { handleChange('vi') }}>{t('header.vietnamese')}</button></li>
                    </ul>
                </div>
            </div>
        </div>
...
function Header() {
    ...
    const [errorMessage, setErrorMessage] = useState(t('header.searchResult'));
            ...
            const products = await productsService
                ...
                .then((productPage: Paginated<Product>) => {
                    ...
                    if (productPage.data[0].name !== undefined) {
                        setFound(true);
                        setAllProducts(productPage.data);
                        setErrorMessage(t('header.errorFound'));
                        setErrorClass("form-control is-valid");
                    }
                    else {
                        setErrorMessage(t('header.error'));
            ...
            <header>
                    ...
                    <div className="container">
                        <div className="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
                            ...
                            <form className="col-12 col-lg-auto mb-2 mb-lg-0 me-lg-auto" onSubmit={handleSearch} noValidate>
                                <input type="search" className="form-control" placeholder={t('header.search')} aria-label="Search" name="search" value={search} required onChange={e => setSearch(e.target.value)} />
                            </form>
                ...
                <table className="table">
                    <thead>
                        <tr>
                            <th scope="col">ID</th>
                            <th scope="col">{t('header.productName')}</th>
                            <th scope="col">{t('header.brand')}</th>
                            <th scope="col">{t('header.quantity')}</th>
                            <th scope="col">{t('header.price')}</th>
                        </tr>
                    </thead>
                    <tbody>
                        {productRows}
